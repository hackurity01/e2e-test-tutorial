...
describe("calculateOrderSummary", () => {
  test("단일/복수 상품 합계를 계산한다", () => {
    ...
    const result = calculateOrderSummary(items, baseContext(), defaultPricingRules);
    expect(result.subtotal).toBe(25_000);
    expect(result.grandTotal).toBe(25_000 + defaultPricingRules.baseShippingFee);
  });

  test("정률 쿠폰은 최대 할인 제한을 적용한다", () => {
    const items: CartLine[] = [{ productId: "p1", unitPrice: 100_000, quantity: 1 }];
  ...
  test("무료배송 임계값은 '할인 적용 후' 기준으로 판단한다 (49,999/50,000 경계)", () => {
    ...
    const items: CartLine[] = [{ productId: "p1", unitPrice: 50_000, quantity: 1 }];
    const coupon = percentCoupon({ percent: 1 }); // 500원 할인 -> 49,500
    const result1 = calculateOrderSummary(items, baseContext({ coupon }), rules);
    expect(result1.shippingFee).toBe(3_000);

    const items2: CartLine[] = [{ productId: "p1", unitPrice: 50_000, quantity: 1 }];
  ...
  test("포인트 사용은 결제금액의 최대 비율(기본 20%) 및 보유 포인트를 초과할 수 없다", () => {
    const rules = { ...defaultPricingRules, maxPointRate: 0.2, baseShippingFee: 0 };
    const items: CartLine[] = [{ productId: "p1", unitPrice: 100_000, quantity: 1 }];
